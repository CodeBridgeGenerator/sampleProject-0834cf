...
import vendorsFakerFactory from "./vendorsFakerFactory";
...
const VendorsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vendorsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vendors").create(elem));
...
export default connect(mapState, mapDispatch)(VendorsFakerDialogComponent);
