...
import incidentsFakerFactory from "./incidentsFakerFactory";
...
const IncidentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = incidentsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incidents").create(elem));
...
export default connect(mapState, mapDispatch)(IncidentsFakerDialogComponent);
